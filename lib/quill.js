...
quill.start = function (callback) {  
  ...
  quill.init(function (err) {
    if (err) {
      quill.welcome();
      callback(err);
      return quill.showError.apply(null, [command[0]].concat(arguments));
    }
    ...
    if (!username && quill.config.get('requiresAuth').indexOf(quill.argv._[0]) !== -1) {
      return quill.commands.users.login(function (err) {
        if (err) {
          callback(err);
          return quill.showError.apply(quill, [command[0]].concat(arguments));
        }
...
quill.exec = function (command, callback) {
  function execCommand (err) {
    ...
    quill.router.dispatch('on', command.join(' '), quill.log, function (err, shallow) {
      if (err) {
        callback(err);
        return quill.showError(command.join(' '), err, shallow);
      }
...
quill.setup = function (callback) { 
  ...
  var client = conservatory.createClient({
    ...
    auth: {
      ...
      password: quill.config.get('password')
    }
  });
  
  flatiron.common.mixin(quill, client);
    ...
    if (err.result) {
      ...
      if (err.result.result && err.result.result.error) {
        if (err.result.result.error.stderr || err.result.result.error.stdout) {
          quill.log.error('');
          quill.log.error('There was an error while attempting to start your application.');
          quill.log.error(err.result.result.error.message);
          if (err.result.result.error.blame) {
            quill.log.error(err.result.result.error.blame.message);
            quill.log.error('');
            quill.log.error('This type of error is usually a ' + err.result.result.error.blame.type + ' error.');
          }
          
          quill.log.error('Error output from your application:');
          quill.log.error('');
        ...
        else if (err.result.result.error.stack && quill.config.get('debug')) {
          quill.log.error('There was an error while attempting to deploy your application.');
          quill.log.error('');
          quill.log.error(err.result.result.error.message);
          ...
          quill.log.error('Error output from Haibu:');
          quill.log.error('');
