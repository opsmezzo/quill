/*
 * files.js: Commands related to packaging relevant nodejitsu files.
 *
...
files.upload = function (container, callback) {
  var env = quill.config.get('env'),
      ...
      systemsDir = path.join(process.cwd(), 'node_modules', 'conservatory', 'node_modules', 'nodejitsu-systems'),
  ...
  fs.readdir(sharedDir, function (err, files) {
    ...
    client.setAuth(function () {
      function uploadFile (file, next) {
        ...
        client.addFile(container, file, path.join(sharedDir, file), function (err, added) {
