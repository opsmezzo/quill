...
// Creates an instance of the `nodejitsu` Rackspace
...
function checkKeyhost (callback) {
  ...
  function extractKeyhost (keys, done) {
    ...
    conservatory.utils.keys.extractHost(keys.public, function (err, name) {
      if (err) {
        return callback(err);
      }
  ...
  rimraf(tmpkey, function () {
    ...
    conservatory.utils.keys.keygen(tmpkey, function (err, private, public) {
      var keys = {
        public: public,
        private: private
      };
...
keys.gethost = function (callback) {
  ...
  conservatory.utils.keys.getHost(true, showKeyhost);
};
