...
keys.downloadall = function (callback) {
  ...
  quill.users.getKeys(function (err, keys) {
    ...
    var authorizedKeys = quill.common.authorizedKeys();
    
    quill.log.info('Downloaded ' + keys.length + ' SSH keys.');
    quill.log.warn('About to overwrite ' + authorizedKeys.magenta);
    quill.prompt.get(['yesno'], function (err, result) {
      ...
      quill.log.warn('Overwriting ' + authorizedKeys.magenta);
      fs.writeFile(authorizedKeys, keys.join('\n\n'), callback);
    })
  });
};
...
keys.download = function (username, callback) {
  ...
  quill.users.getKeys(username, function (err, keys) {
    ...
    quill.log.info('Downloaded ' + keys.length + ' SSH keys.');
    quill.log.warn('Appending keys to ' + authorizedKeys.magenta);
    quill.common.appendKeys(keys, callback);
  });
};
