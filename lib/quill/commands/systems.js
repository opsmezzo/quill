...
systems.lifecycle = function (script) {
  var systems  = Array.prototype.slice.call(arguments, 1),
      watch    = quill.argv.watch || quill.argv.w,
  ...
  //
  // Helper function for starting watching changes
  // to installed systems.
  //
  function watchLifecycle() {
    quill.log.info('quill listening for changes on ' + script.yellow);
    return composer.watch.start(script);
  }

  if (watch && !systems.length) {
...
systems.publish = function (target, callback) {
  ...
  function tryBump(dir, next) {
    if (!quill.argv.bump) {
      ...
      return composer.readJson(dir, next);
    }
  ...
  function publishDir(dir, done) {
    ...
    async.waterfall([
      function bumpAndRead(next) {
        tryBump(dir, function (err, system) {
          return !err
            ? next(null, system)
            : next(err);
        });
      },
      function extractAndPack(system, next) {
        composer.config.extract(system, dir, function (err, vars) {
          if (err) {
            return next(err);
          }
      ...
      async.map(
        ...
        function (err, results) {
          ...
          async.forEach(
            ...
            function (dir, next) {
              publishDir(dir, function (err) {
                if (err) {
                  quill.log.warn('Error publishing: ' + dir.yellow);
                  quill.log.error(err.result && err.result.message || err.message);
                }
...
systems.view = function (name, callback) {
  ...
  quill.systems.get(name, function (err, system) {
    ...
    ['_id', '_rev', '_attachments', 'ctime', 'mtime', 'resource', 'versions']
...
systems.bump = function (release, dir, callback) {
  ...
  fs.readFile(systemJson, 'utf8', function (err, data) {
    ...
    fs.writeFile(systemJson, JSON.stringify(data, null, 2), 'utf8', function (err) {
