...
function lifecycleCommand(script) {
  ...
  //
  // Usage for `quill systems bootstrap`.
  //
...
//
// Define aliases for the main lifecycle scripts: 
// `bootstrap, image, update, teardown`.
//
lifecycleCommand('bootstrap');
lifecycleCommand('image');
lifecycleCommand('update');
lifecycleCommand('teardown');

//
// ### function bootstrap (script, targets, ...callback)
// #### @script {string} Name of the lifecycle script to run.
// #### @targets {Array} List of systems to bootstrap.
...
systems.lifecycle = function (script) {
  ...
  quill.log.info('Executing lifecycle script ' + script.yellow);
