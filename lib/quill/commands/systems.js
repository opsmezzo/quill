...
systems.publish = function (target, callback) {
  ...
  //
  function bump(dir, next) {
    var config = quill.config.get('bump');
    if (config === true) {
      config = 'patch';
    }
...
// #### @name {string} Release type. Defaults to "patch".
// #### @callback {function} Continuation to respond to when complete.
//
...
systems.bump = function (release, dir, callback) {
  ...
  //
  var systemJson = path.join(dir || process.cwd(), 'system.json');
