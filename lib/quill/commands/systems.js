...
systems.publish = function (target, callback) {
  ...
  function publishDir() {
    ...
    composer.package(target, stream, function () { }).on('list', function (json) {
      name = [json.name, json.version].join('@').magenta;
    ...
    stream.on('pipe', function () {
      ...
      composer.publish(system, stream, function (err) {
        return err ? callback(err, true) : callback();
      });
    });
  }
...
systems.unpublish = function () {
  ...
  if (args.length === 0) {
    return callback(new Error('At least one system name is required'), true, true);
  }
