...
servers.move = function (role, target, callback) {
  ...
  conservatory.resources.Server.forRole(role, function (err, servers) {
    function moveServer (server, next) {
      ...
      conservatory.resources.Server.destroy(server._id, function (err, result) {
        var clone = quill.log.clone(server.properties);
        clone.role = target;
        clone._id = target + '_' + server.addresses.public[0];
        delete clone._rev;

        conservatory.resources.Server.create(clone, function () {
          next();
        });
      });
    }
