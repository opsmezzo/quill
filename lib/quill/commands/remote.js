    ...
    quill.remote.get(name, function (err, remote) {
      ...
      quill.remote.destroy(name, function () {
        ...
        Object.keys(data).forEach(function (key) {
          var valueType = typeof data[key],
              targetType = typeof remote.settings[key];

          //
          // Support one level of nesting merges.
          // TODO: Should this be a full merge or an option?
          // This now does a very very ugly 3 level nesting merge
          // TODO: Make this not terrible
          //
          if (valueType === 'object' && targetType === 'object'
              && !Array.isArray(data[key])
              && !Array.isArray(remote.settings[key])) {
            Object.keys(data[key]).forEach(function (nested) {
              var valType = typeof data[key][nested];
              var targType = typeof remote.settings[key][nested];

              if (valType === 'object' && targType === 'object'
                  && !Array.isArray(data[key][nested])
                  && !Array.isArray(remote.settings[key][nested])) {
                Object.keys(data[key][nested]).forEach(function(second) {
                  var vType = typeof data[key][nested][second],
                      tType = typeof remote.settings[key][nested][second];

                  if(vType === 'object' && tType === 'object'
                      && !Array.isArray(data[key][nested][second])
                      && !Array.isArray(remote.settings[key][nested][second])) {
                    Object.keys(data[key][nested][second]).forEach(function (third) {
                      remote.settings[key][nested][second][third] = data[key][nested][second][third];
                    });
                  }
                  else {
                    remote.settings[key][nested][second] = data[key][nested][second];
                  }
                });
              }
              else {
                remote.settings[key][nested] = data[key][nested];
              }
            });
          }
          else {
            remote.settings[key] = data[key];
          }
        });
