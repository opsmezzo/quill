/*
 * ignore.js: Helper functions for parsing ignore files.
 *
 * Copyright 2009, 2010, 2011 Isaac Z. Schlueter (the "Author")
 *
 * Modified by Nodejitsu Inc., 2012 under MIT
 *
 */
...
//
// To start off with, ~/.{quill,git}ignore is added, as is
// prefix/{quill,git}ignore, effectively treated as if they were in the
...
function addIgnoreFile (file, gitBase, list, dir, cb) {
  ...
  parseIgnoreFile(file, gitBase, dir, function (er, data) {
    if (!er && data) {
      // system.json "files" array trumps everything
...
function test (file, excludeList) {
  if (path.basename(file) === "system.json") return true
  var incRe = /^\!(\!\!)*/
    , excluded = false
