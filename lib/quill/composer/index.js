...
//
common.mixin(composer, require('./tar'))
common.mixin(composer, require('./dependencies'));
common.mixin(composer, require('./validate'));
...
composer.install = function (systems, callback) {
  ...
  function installAll(runlist) {
    async.forEachSeries(runlist, composer.installOne, function (err) {
      return err
        ? callback(err)
  ...
  composer.download(runlist, function (err) {
    return err
      ? callback(err)
      : installAll(runlist);
  });
...
composer.installOne = function (system, callback) {
