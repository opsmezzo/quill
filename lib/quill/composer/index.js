...
//
composer.runlist = function (systems) {
...
composer.validateSystem = function (system, os) {
...
composer.validateOs = function (system, os) {
  if (!os) {
    return;
  }
...
function osRunlist(system, os) {
  ...
  if (!system.os[os].dependencies && !system.os[os].runlist) {
    return systemNames(system.os[os]).map(function (parts) {
      return parts.join('@');
    });
  }
}