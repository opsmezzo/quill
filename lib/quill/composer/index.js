...
composer.dependencies = function (systems, callback) {
  ...
  function updateTree(parts, next) {
    ...
    quill.systems.get(name, function (err, system) {
      ...
      composer.dependencies(system.dependencies, function (err, subtree) {
        if (err) {
          return next(err);
        }
  ...
  async.forEach(systemNames(systems), updateTree, function (err) {
    return err
      ? callback(err)
      : callback(null, tree);
  });
...
//
composer.runlist = function (systems, list, callback) {
  ...
  //
  // Helper function which builds the subtree for 
  // `name` and inserts it into the parent `list`.
  //
  function updateList(parts, next) {
    ...
    quill.systems.get(name, function (err, system) {
      ...
      if (list.length) {
        //
        // TODO: Filter existing systems and raise errors for 
        // mismatched semver dependencies.
        //
        list.unshift(tag);
      }
      else {
      ...
      deps = system.runlist.reverse().map(function (name) {
  ...
  async.forEachSeries(systemNames(systems), updateList, function (err) {
    return err
      ? callback(err)
      : callback(null, list);
  });
