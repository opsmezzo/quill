...
exports.actions = {
  configure: {
    ...
    pre: [
      //
      // Ensure that fresh copies of our templates
      // and moved into place in for the installed
      // system 
      //
      function rewriteTemplates(system, next) {
  ...
  update: {
    pre: [
      //
      // Ensure that if a newer version of the system that
      // satisfies parent dependencies it is downloaded. 
      //
...
exports.runOne = function (script, system, config, callback) {
  ...
  async.series([
    //
    // 1. Refresh templates from the cache since they are
    // rendered inline
    //
    function refreshTemplates(next) {
