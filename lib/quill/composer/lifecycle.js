...
exports.actions = {
  ...
  update: {
    pre: [
      ...
      //
      function checkVersion(system, config, done) {
        quill.systems.get(system.name, function (err, latest) {
          ...
          async.waterfall([
            ...
            //    version.
            //
            function updateInstalled(systems, next) {
              composer.installed.remove([system.name, system.version].join('@'), function (err) {
                return !err
                  ? composer.installed.add(systems, next)
                  : next(err);
              });
            },
            //
      ...
      //
      function checkDependencies(system, config, done) {
        ...
        async.waterfall([
          async.apply(async.parallel, {
            ...
            //
            installed: async.apply(quill.installed.list),
            //
            ...
            //
            latest: async.apply(
              quill.dependencies.maxSatisfying,
              system
            )
          }),
          //
          ...
          function tryUpdate(result, next) {
            ...
            if (update.length) {
              //
              // TODO: Actually update the installed ish.
              //
            }
  ...
  function runAll(runlist, done) {
    async.forEachSeries(
      ...
      function runOne(system, next) {
        system.config = common.mixin(remoteDependencies || {}, system.config || {});
    ...
    async.waterfall([
      ...
      function maybeLocalizeRunlist(installed_, next) {
        ...
        return /^install/.test(action) && quill.argv.force
          ? next(null, runlist)
          : next(null, composer.runlist.localize(runlist, installed));
      },
      composer.installed.add
    ], function (err) {
      return !err
        ? runAll(composer.runlist.filter(action, runlist, installed), done)
        : done(err);
    });
  }
