...
exports.dependencies = function (systems, os, callback) {
  ...
  function updateTree(parts, next) {
    ...
    quill.systems.get(name, function (err, system) {
      ...
      system.dependencies = common.mixin(
        system.dependencies || {},
      ...
      async.parallel({
        dependencies: async.apply(
          composer.dependencies,
          system.dependencies
        ),
        remoteDependencies: function remoteDeps(next) {
          return system.remoteDependencies
            ? composer.dependencies(system.remoteDependencies, next)
            : next();
        }
...
//
exports.remoteRunlist = function (options) {
  var os       = options.os,
      list     = options.list  || [],
      depth    = options.depth || 0,
      maxDepth = options.maxDepth,
...
//
// ### function verifyRemoteRunlist (options, callback)
...
// remote `runlist`.
//
exports.verifyRemoteRunlist = function (options, callback) {
  var runlist = options.runlist;

  if (!runlist || !runlist.length || !quill.remote.servers) {
    return callback();
  }

  quill.remote.servers(function (err, config) {
    if (err) {
      return callback(err);
    }
...
exports.maxSatisfying = function (system, callback) {
  async.waterfall([
    ...
    function satisfyingSet(os, next) {
      ...
      exports.dependencies(parent, os, function (err, dep) {
        if (err) {
          return next(err);
        }
