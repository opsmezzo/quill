...
//   * If the system exists in the `list` validate the version.
//     * If there is a conflict, respond with error.
// 3. Recursively add the runlist of each of the `remoteDependencies` to the list.
//
exports.remoteRunlist = function (options, callback) {
  var os       = options.os,
      list     = options.list  || [],
      depth    = options.depth || 0,
      maxDepth = options.maxDepth,
  ...
  function updateList(parts, next) {
    ...
    quill.systems.get(name, function (err, system) {
      ...
      if ((!system.runlist || !system.runlist.length) && system.remoteDependencies) {
        system.runlist = Object.keys(system.remoteDependencies);
      }
      ...
      deps = (deps || []).concat((system.runlist || []).map(function (name) {
        return system.remoteDependencies[name]
          ? [name, system.remoteDependencies[name]].join('@')
          : null;
      })).reverse().filter(Boolean);
