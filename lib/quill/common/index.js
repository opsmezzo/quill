...
// #### @options Object to add `.meter` to.
//
// Adds a `multimeter` instance to the `options` if
//   * One does not already exist
//   * quill is running in in a TTY
//   * `raw` is not true.
//
common.meter = function (options) {
  if (!options.meter && !quill.config.get('raw') && quill.config.get('progress')
      && process.stdout.isTTY) {
    options.meter = multimeter(process);
    options.meter.charm.cursor(false);
  }
